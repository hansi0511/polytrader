...
async function getKlines(limit) {
  ...
  return candles.slice(-limit).map(function(c) {
    return [c[0]*1000, c[1], c[2], c[3], c[4], c[6], (c[0]+60)*1000, (parseFloat(c[5])*parseFloat(c[6])).toString(), c[7], '0','0','0'];
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    if (sub.includes('klines')) {
      return res.status(200).json(await getKlines(parseInt(req.query.limit) || 100));
    }
    if (sub.includes('ticker')) {
      return res.status(200).json(await getTicker24hr());
    }
    return res.status(404).json({ error: 'Unknown endpoint', path: sub });
  } catch(err) {
